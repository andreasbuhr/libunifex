...
struct _with_type_erased_tag_invoke<
  ...
  struct type {
    friend Ret tag_invoke(_overload::base_cpo_t<CPO> cpo, replace_this_t<Args, Derived>... args)
        noexcept(NoExcept) {
      using cpo_t = _overload::base_cpo_t<CPO>;
      static_assert(
      ...
      // The strange cast of the cpo to save the compiler from having to reserve
      // space in the image for the cpo in case we pass it by reference through
